...
function seedArea(cb) {
  ...
  for (var i = 0; i < rangers.length; i++) {
    requests.push(Promise.resolve(_jquery2['default'].ajax({
      url: baseUrl + '/rangers',
      type: 'POST',
  ...
  for (var i = 0; i < wombats.length; i++) {
    requests.push(Promise.resolve(_jquery2['default'].ajax({
      url: baseUrl + '/wombats',
      type: 'POST',
...
function showError(err) {
  (0, _jquery2['default'])('#error').html(JSON.stringify(err));
}

function updateDisplay() {
  var me = arguments.length <= 0 || arguments[0] === undefined ? "" : arguments[0];

  return Promise.resolve(_jquery2['default'].ajax({
    url: baseUrl + '/areamap/' + me,
    type: 'GET',
    cache: false
  }).then(function (data) {
    (0, _jquery2['default'])('#map').empty();
    _jquery2['default'].each(data, function (key, val) {
      (0, _jquery2['default'])('#map').append(key + ' ' + val + '\n');
    });
  }));
}
